...
QVector<wave> SFT::fourier(QVector<float> data, int freq)
{
    ...
    for (int N = 0; N < freq; N++){  // Cn的n 頻率
        ...
        for (int t = 0; t < x_axis; t++) {
            ...
            float time =
                ((float) t /
                 (float) x_axis);  //走了總步數的比例，就是走了圓周的多少百分比
    ...
    // for(auto w:result)
    // qDebug()<<"re:"<<w.re<<" im:"<<w.im<<" freq:"<<w.freq<<" amp:"<<w.amp<<"
    // phase:"<<w.phase;
    return result;
}
QVector<QVector<wave>> SFT::fourier2D(QVector<QVector<float>> data,
                                      int freqx,
                                      int freqy)
{
    qDebug() << data;
        ...
        for (int fx = 0; fx < freqx; fx++) {
            for (int y = 0; y < y_axis; y++) {
                for (int x = 0; x < x_axis; x++) {
                    float rate = (-1 * M_PI_2);
                    float timex =
                        ((float) x / (float) x_axis) *
                        (float)
                            fx;  //走了總步數的比例，就是走了圓周的多少百分比
...
QVector<float> SFT::reverse(QVector<wave> data)
{
    ...
    for (int time = 0; time < 180; time++) {
        float intensity = 0;
        for (auto w : data) {
            // intensity+=w.amp*(w.re*qCos(M_PI_2*w.freq*(float)time/180) -
            // w.im*qSin(M_PI_2*w.freq*(float)time/180));
            intensity += w.amp * qCos(1 * M_PI_2 * w.freq * (float) time / 180 +
                                      qAtan2(w.im, w.re));
        }
